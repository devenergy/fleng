...
Auth.prototype.configureProvider = function configureProvider(options, name) {
  ...
  var authPath = options.authPath != null ? options.authPath || '/auth/' + name : void 0;
  var callbackPath = options.callbackPath != null ? options.callbackPath || '/auth/' + name + '/callback' : void 0;
  ...
  if (authType === 'local') {
    ...
    if (authPath) {
      router.get(authPath, passport.authenticate(name, {scope: options.scope}));
    }
